...
| 機能 | 内容 | 実行頻度（デフォルト） |
| --- | --- | --- |
| 自動投稿 | 季節・流行を踏まえた集客投稿を生成し公開 | 1日30回・朝昼夜のピーク重視・投稿時刻は毎日変動 |
| AI返信 | 投稿へのコメントに広報担当として自然に返信 | 約2時間ごと |
| トークン更新 | Threadsの長期トークンを自動更新 | 毎週月曜 |

> 自動投稿は「10分ごとに起動して、その日の投稿予定時刻になったら1件投稿」する方式です。予定時刻はその日の日付から決まるため**毎日少しずつ変わり**、昼12時台・夜20〜22時台などエンゲージメントの高い時間帯に多く配分されます（`src/schedule.ts`）。本文の長さも投稿ごとに **30〜120文字** の範囲でばらつきます。
