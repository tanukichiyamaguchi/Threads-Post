...
| 自動投稿 | 素材＋学習知見から『バズる×蒲田×美容』の投稿を生成し公開 | 1日40回・朝昼夜のピーク重視・投稿時刻は毎日変動 |
| 投稿改善（バズ学習） | 投稿実績を分析＋バズる投稿の型をWeb検索で調べ、投稿方針を毎日更新 | 毎日1回（05:00 JST） |
| AI返信 | 投稿へのコメントに広報担当として自然に返信 | 約2時間ごと |
| トークン更新 | Threadsの長期トークンを自動更新 | 毎週月曜 |

### 毎日改善する仕組み（バズ学習ループ）

`improve` が毎日、次の2つを行い、**バズる投稿の型**を学習して `state/learnings.json` に保存します。各投稿の生成時にこの知見を注入するので、**投稿は日々改善**されていきます。

1. **自店の実績分析** … 最近の投稿のいいね・返信・リポスト・引用・閲覧数を取得し、伸びた／伸びなかった投稿の傾向を抽出。
2. **外部のバズ調査** … Web検索で、20〜40代女性・美容・ローカルで実際に伸びた投稿の「型」を調査。
...
- **バズ学習は1日1回**（`improve`）。実績分析＋Web検索で `state/learnings.json` を更新します。
- **各投稿は素材＋学習知見から本文だけを安価に生成**（Web検索なし・思考オフ・effort低）。1投稿あたり数秒・低コストです。
...
npm run improve        # 投稿実績を分析＋バズ調査して学習知見を更新（1日1回でOK）
...
1. **リサーチ（2週に1回）** … Claude が Web検索で公式情報・季節・蒲田ローカルを調査 → 投稿素材（アングル集）を `state/content-plan.json` に保存。
2. **バズ学習（1日1回）** … 最近の投稿の実績を取得＋バズる投稿の型をWeb検索で調査 → 学習知見を `state/learnings.json` に保存。
3. **投稿（5分ごと）** … 予定時刻なら素材からアングルを選び、学習知見を注入して本文だけを安価に生成 → Threadsへ公開 → 履歴を保存。
4. **返信（2時間ごと）** … 自分の最近の投稿の会話を取得 → 新着コメントを抽出（自分・空・処理済みは除外）→ Claude が返信を生成 → コメントへ返信。
5. 実行状態（`state/`）は Actions が自動でコミットし、次回以降に引き継ぎます。
