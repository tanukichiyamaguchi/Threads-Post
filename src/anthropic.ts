...
export async function buildLearnings(
  ...
  const user = [
    ...
    `- todayTopics: 本日のインプが狙える話題を6〜10個。美容・共感・蒲田の日常のいずれかで自然に扱える話題を選ぶ。無理に美容やまつげ・眉に結びつけず、話題単体で共感が取れるならそのままでよい（目的を分ける・こじつけない）。不謹慎・炎上リスクのある話題は除外する。`,
...
function buildPostSystemPrompt(): string {
  ...
  return [
    ...
    `4. 投稿の目的を無理に混ぜない。美容の話は美容として、地元・共感の話はそれ単独で成立させる。ローカルネタや共感ネタを強引にまつげ・眉や来店の話に着地させない。宣伝は一切しない。`,
    ``,
    ...
    `- 美容そのものへの偏愛と、蒲田・大田区への愛着が、押し付けなく言葉の端に滲む温度を保つ（豆知識の羅列で終わらせない）。`,
    ...
    `- 最優先はインプレッション（表示回数）。共感・保存・返信・リポストされやすい中身にする。美容がテーマの回だけ専門家の知見を効かせ、美容と関係ないテーマを無理にまつげ・眉や店の話に結びつけない（目的を分ける）。`,
