...
export async function researchNotes(today: string): Promise<string> {
  const system =
    ...
    "指定店舗の公式情報（メニュー・クーポン・価格）を確認し、今後2週間の『バズる美容の投稿』に役立つ事実・季節・生活文脈の切り口を整理します。";
  ...
  const user = [
    ...
    ``,
    `# 手順3: 読者の生活文脈（美容の話に添える背景として使う）`,
    `大田区に住む・通う20〜40代女性の生活感（共働き・子育て・忙しさ・コスパ志向・自分時間の少なさ・通勤や支度の時間帯など）を集めてください。`,
    `あわせて、この地域の気候や環境で美容に影響しうる要素（湿度、風、花粉、通勤距離、屋根付き商店街で雨をしのげる等）があれば拾ってください。`,
    `※ここで集めるのは、あくまで美容の投稿に添える生活文脈です。グルメ・名所・観光・銭湯・街あるあるそのものは投稿テーマにしないため、収集対象外。`,
    `最新情報でなくてよく、地元の人が実感として分かる定番の生活感でOK。失礼なステレオタイプは避ける。`,
    ``,
    `# 出力`,
    `(A) 当店のメニュー・特徴・アクセス・営業（確認できた事実のみ）`,
    `(B) クーポン（上位5件まで・名称/内容/価格/条件。確認できたもののみ。無ければ「確認できず」と明記）`,
    `(C) 今後2週間の季節・話題の切り口（5〜8個、各1〜2行。肌・髪・メイク・目元など美容への影響に結びつく形で）`,
    `(D) 読者の生活文脈の素材（20〜40代女性の生活感・時間帯・気候や環境が美容にどう効くかの切り口を8個以上。ローカルの名所やグルメではなく、美容に接続する生活文脈を集める）`,
  ].join("\n");
      ...
      items: {
        ...
        properties: {
          category: {
            type: "string",
            description:
              "美容サブジャンル: eyes（まつげ・眉・目元）/ skin / makeup / hair / habit / season / mindset のいずれか。美容以外のカテゴリは使わない。",
          },
...
export async function buildContentPlan(
  ...
  const user = [
    ...
    ``,
    `# 依頼`,
    `上記リサーチをもとに、今後2週間ぶんの投稿素材を作成してください。最優先は『バズる美容の投稿』で20〜40代女性の認知を広げること（来店誘導は目的にしない）。`,
    ...
    `   重要: 全てのアングルは美容の話にすること。美容以外を主題にしたアングル（街やエリアのあるある、グルメ、交通、観光、銭湯、美容と無関係な日常ネタ）は1つも作らない。`,
    `   category は次の美容サブジャンルから選び、特定ジャンルに偏らせない:`,
    `     eyes（まつげ・眉・目元）／skin（スキンケア・肌の悩み）／makeup（メイク）／hair（ヘア・髪の悩み）／habit（美容習慣・セルフケア・道具）／season（季節と肌や髪の変化）／mindset（美容の思い込み崩し・垢抜け・自分磨き）`,
    `   eyes は全体の3割まで（13個程度が上限）。まつげ・眉に偏らせず、skin・makeup・hair・habit・season・mindset を厚めに配分する。`,
    `   蒲田・大田区の要素は、美容の話に添える生活文脈としてなら使ってよい（通勤前の支度、湿度、風の強い日など）。ただしエリアやローカルそのものが主題のアングルは作らない。`,
    `## 大田区ローカルの背景（美容の話に添える生活文脈としてのみ使う。ローカルを主題にしない）`,
    brand.localContext,
    `## 内容配分`,
    brand.contentMix,
    `## 目的`,
...
export async function researchViralPlaybook(today: string): Promise<string> {
  ...
  const user = [
    ...
    `目的: 『バズる美容の投稿』で"認知"を広げること（来店誘導が目的ではない）。投稿テーマは美容のみ。`,
    ``,
    ...
    `2) 美容ジャンルで保存・返信されやすいネタの型。まつげ・眉・目元だけでなく、スキンケア・肌悩み・メイク・ヘア・美容習慣・季節と肌の変化・垢抜けまで幅広く。`,
    `3) 美容の中でもジャンル別（肌／髪／メイク／目元）にどんな切り口が伸びやすいかの違い。`,
    `4) 逆に伸びにくい・宣伝臭くて避けるべき型。`,
    `※特定のバズ投稿を丸写しするのではなく、応用できる「型・原則」を抽出する。`,
    `※当アカウントは美容のみを投稿する。ローカル・グルメ・街ネタの型は調査対象外。`,
    ``,
    `# 出力`,
    `箇条書きで、(1)バズる型・原則 (2)美容ジャンルで効くネタ (3)美容ジャンル別の切り口の違い (4)避けるべき型 をそれぞれ具体的に、多めに。`,
  ].join("\n");
...
const VIRAL_PLAYBOOK_SCHEMA = {
  ...
  properties: {
    ...
    viralAngles: {
      type: "array",
      items: { type: "string" },
      description: "美容（20〜40代女性向け）で普遍的にバズを狙える切り口アイデア。10〜16個。まつげ・眉に偏らせず、肌・髪・メイク・美容習慣まで幅広く。",
    },
  },
...
export async function buildViralPlaybook(
  ...
  const user = [
    ...
    ``,
    `# 依頼`,
    `上記を、今後ずっと使えるバズる投稿のプレイブックへ整理してください（20〜40代女性向け・美容のみを投稿する方針・認知拡大が目的）。`,
    `- playbook: バズる型・原則（書き出し／構成／締め方／避けるべき型を含む）。12〜18個、具体的で実行可能に。`,
    `- viralAngles: 美容で普遍的にバズを狙える切り口アイデアを10〜16個。まつげ・眉に偏らせず、肌・髪・メイク・美容習慣・季節と肌の変化まで幅広く散らす。ローカル・グルメ・街ネタの切り口は作らない。`,
...
export async function researchTrendingTopics(today: string): Promise<string> {
  ...
  const user = [
    `本日: ${today}（日本・東京）。`,
    `発信者: まつげ・眉を軸に美容を教えてきた専門家（美容に詳しい等身大の発信者）。投稿テーマは美容のみ。`,
    `対象読者: 20〜40代の女性（蒲田・大田区に住む／働く人が中心）。目的は認知拡大（来店誘導ではない）。`,
    ``,
    `# 依頼`,
    `web_search で、今日〜ここ数日で「インプレッションが取れそう・話題になっている」ネタを集めてください。`,
    `A) 全国的なトレンド・季節・話題（ニュース、行事、天気、SNSで話題の事柄、美容・ライフスタイル系の流行など）。`,
    `B) 美容に直結する季節・気候の話題（乾燥、湿度、花粉、紫外線、冷房や暖房による肌や髪の変化など）。大田区のローカルな気候・生活シーンが絡められるものがあれば添えてよいが、ローカルのイベントや街ネタそのものは対象外。`,
    `※政治・災害・事故・訃報など、美容アカウントが便乗すると不謹慎・炎上しうる話題は除外する。`,
    `※重要: 集めるのは美容の投稿として成立する話題のみ。美容と無関係な話題（グルメ、交通、観光、街のイベント）は、どれだけ話題性があっても採用しない。`,
    `※各話題は、20〜40代女性の美容の悩みや行動にどう自然につながるかを一言添える。こじつけが必要なものは出さない。`,
    `※重要: Web検索で実際に確認できた話題だけを出すこと。検索できなかった・確認できなかった場合は、`,
    `推測や一般知識で話題を作らず「話題を確認できませんでした」とだけ出力する（事実誤認の投稿を防ぐため）。`,
    ``,
    `# 出力`,
    `箇条書きで6〜10件。各行「話題 → 美容の投稿としてどう扱うか（肌・髪・メイク・目元など具体的に）」。`,
  ].join("\n");
...
const LEARNINGS_SCHEMA = {
  ...
  properties: {
    ...
    todayTopics: {
      ...
      items: { type: "string" },
      description:
        "その日のインプが狙える話題＋美容の投稿としての扱い方。6〜10個。美容の投稿として成立する話題のみ（美容と無関係な話題・ローカルネタ単体は除外）。不謹慎・炎上リスクのある話題は除外。",
    },
  },
...
export async function buildLearnings(
  ...
  const user = [
    ...
    ``,
    `# 依頼`,
    `上記から、本日の『バズる美容の投稿』（20〜40代女性向け・美容のみ・認知拡大が目的）を改善するための知見を作成してください。`,
    ...
    `- todayTopics: 本日のインプが狙える話題を6〜10個。ただし全て美容の投稿として成立する話題に限る（美容と無関係な話題・ローカルネタ単体は採用しない）。まつげ・眉に限らず、肌・髪・メイク・季節と肌の変化・美容習慣など美容全般から選び、まつげ・眉に偏らせない。こじつけが必要な話題は最初から採用しない。不謹慎・炎上リスクのある話題は除外する。`,
...
export async function researchEnvironmentTrends(today: string): Promise<string> {
  ...
  const user = [
    `本日: ${today}（日本）。`,
    `発信者: まつげ・眉を軸に美容を発信するアカウント（20〜40代女性向け・テキスト投稿のみ・投稿テーマは美容のみ・目的はインプレッション最大化）。`,
    ``,
    `# 依頼`,
    ...
    `3) 美容ジャンル内（肌・髪・メイク・目元）でネタや切り口がどう移り変わっているか`,
...
const HOOK_GUIDE: Record<string, string> = {
  ...
  自己開示: "自分の本音・体験・気づきから入る（美容をよく知る一人としての等身大の一人称で。店舗スタッフや現役施術者を装った当事者トークにはせず、宣伝にもしない）",
};
...
export async function generatePost(
  ...
  const ctaBlock = includeCta
    ...
    : `# 予約・宣伝について\nこの投稿では予約・来店の呼びかけ（ご予約・プロフィールのリンク等）は入れない。売り込まない。目的は美容の投稿での認知拡大。共感・気づき・あるあるに徹する。`;

  const kamataBlock = arms.kamata
    ? `# エリアの言及\nこの投稿は美容の話が主題。そのうえで、美容の場面の背景として地元のエリアを自然に1回だけ添える（例: 通勤前、湿度、風の強い日など、その土地の生活感が美容の悩みや行動に結びつく形）。蒲田だけに固定せず、3km圏内の周辺エリア（大森・糀谷・六郷・矢口・下丸子・池上・千鳥町など）や大田区全体（田園調布・洗足池・久が原・雪谷・羽田・馬込など）も使ってよい。エリアの紹介・グルメ・街あるあるが主題になったら失敗。`
    : `# エリアの言及\nこの投稿ではエリアには言及しない（全国の読者に刺さる美容の内容にする）。`;
  ...
  const user = [
    ...
    `美容がテーマの回は専門家ならではの視点を効かせる。美容と関係ないテーマを無理にまつげ・眉や店の話に結びつけない（目的を分ける）。売り込み・宣伝臭は一切出さない。`,
...
function buildPlanSystemPrompt(): string {
  return [
    ...
    `美容に詳しく信頼感のある発信者として、美容のみをテーマにした投稿で20〜40代女性の認知を広げる素材を設計します。`,
    ``,
    `## 目的`,
    brand.goal,
...
function buildLearningsBlock(
  ...
  const blocks = [
    ...
    sec(
      "実際に伸びた自店の投稿（具体性の出し方など話の運び方だけを参考にする。書き出し・締め方・構成・言い回しの流用は禁止。過去に美容以外の投稿が含まれていてもテーマは真似しない。今後の投稿は必ず美容にする）",
...
function buildPostSystemPrompt(): string {
  ...
  return [
    `あなたは「${brand.name}」（${brand.area}の${brand.businessType}）の${brand.persona}です。`,
    `Threadsで『バズる美容の投稿』を作り、インプレッション（views）を最大化して20〜40代女性に広く認知されることを狙います。`,
    `売り込みではなく、思わず反応したくなる美容の共感・気づき・あるあるで拡散を狙う発信者です。`,
    ...
    `3. あなたは店舗スタッフや広報ではなく、まつげ・眉を軸に美容を教えてきた専門家（アイリストを育てる立場）。深い知見と美容への愛が、押し付けなく言葉の端に滲む。無味乾燥な他人事の説明で終わらせない。`,
    `4. 投稿のテーマは必ず美容にする。美容以外を主題にした投稿は作らない。宣伝は一切しない。`,
    ``,
    `## 投稿テーマの範囲（美容のみ・最重要）`,
    `- 全ての投稿は美容の話にする。一行目を読んで美容の話だと分からなければ、その時点で失敗作。`,
    `- 美容以外を主題にした投稿は作らない。禁止例: 街やエリアのあるある、グルメ・飲食店、交通や路線のネタ、銭湯や観光、天気そのものの話、美容と無関係な日常あるある。`,
    `- ただし美容はまつげ・眉に限らない。スキンケア・肌の悩み・メイク・ヘア・季節と肌や髪の変化・美容習慣やセルフケア・美容の思い込み崩し・垢抜け・自分磨きまで広く扱う。`,
    `- まつげ・眉に偏らせない。毎回まつげ・眉の話にすると同じ投稿ばかりになる。目元・眉を主題にするのは全体の3割程度までにし、それ以外は肌・髪・メイク・習慣など別の美容テーマにする。`,
    `- 地元やローカルの要素は、美容の話に添える生活文脈としてだけ使ってよい（例: 通勤前の支度、湿度で髪がうねる、風が強い日の前髪）。エリアやローカルそのものが主題になったら失敗。`,
    `- 美容と無関係な話題を無理やり美容に着地させるこじつけもしない。最初から美容の話題を選ぶ。`,
    ``,
    `## 熱意（あなたの核・ただし宣伝や自分語りにはしない）`,
    `- あなたはまつげ・眉を軸に美容を教えてきた専門家。表面的な紹介ではなく、深く分かっている人ならではの視点・こだわりで書く。`,
    `- 美容を語るときは、まつげ1本・眉1mm・毛流れ・左右差のような、プロの目線でしか見えない面白さ・奥深さを短い一言に滲ませる。`,
    `- まつげ・眉だけでなく、肌・髪・メイクまで含めた美容そのものへの偏愛が、押し付けなく言葉の端に滲む温度を保つ（豆知識の羅列で終わらせない）。`,
    `- 熱意は長い語りではなく短い一撃で。感情が自然にこぼれる一文で言い切る（ダラダラ熱く語らない・大げさなポエムにしない）。`,
    `- 熱意を宣伝にしない。上手さ自慢・来店誘導・効果の保証は熱意ではなく営業。純粋な好きとして自然に出す。`,
    `- 店舗スタッフや現役の施術者を装わない。施術してる・うちのサロンで といった当事者ぶった言い回しはしない。自分の立場や肩書き（教育者である 等）も投稿でわざわざ説明・開示しない。`,
    ``,
    `## 目的`,
    brand.goal,
    `## ターゲット`,
    ...
    `- 最優先はインプレッション（表示回数）。共感・保存・返信・リポストされやすい中身にする。テーマは必ず美容の範囲内から選び、専門家の知見を自然に効かせる（押し付けない）。`,
    `- 読者は20〜40代の女性を想定するが、女性向けであることは書かない。女性の皆さん・女子必見・女性限定のような性別の呼びかけやラベルは使わない。誰に向けるかを宣言するのではなく、描く場面や悩みの具体性でその層に届かせる。男性向けの訴求もしない。`,
    ...
    `- エリア言及の指示がある回も、主題はあくまで美容。エリアは美容の話が起きる場面の背景として一度だけ添える。蒲田だけでなく3km圏内の周辺エリアや大田区全体も使ってよい。事実に反することは書かない。`,
    `- 絵文字は${r.emoji}。`,
...
function buildReplySystemPrompt(): string {
  ...
  return [
    ...
    `## ルール`,
    `- ${r.style}`,
    `- あなたは店舗スタッフや広報botではなく、まつげ・眉を軸に美容を教えてきた専門家。相手の話に、美容への愛情や親しみが自然に滲む温かい一言を添える（売り込み・来店誘導や、自分の立場の説明はしない。当事者ぶった店舗トークにもしない）。`,
